...
test.describe('Playlist Management', () => {
  test('should create a new playlist', async () => {
    ...
    const playlist = await page.locator('[data-testid="playlist-Test Playlist"]');
  ...
  test('should add songs to playlist', async () => {
    ...
    const songTitles = await page.locator('[data-testid="song-title"]').allTextContents();
    ...
    const playlistSongs = await page.locator('[data-testid^="playlist-song-"]').count();
    expect(playlistSongs).toBe(2);
    
    const playlistSongTitles = await page.locator('[data-testid="playlist-song-title"]').allTextContents();
  ...
  test('should remove songs from playlist', async () => {
    ...
    const songTitle = await page.locator('[data-testid="song-title"]').first().textContent();
    await TestHelpers.addToPlaylist(page, songTitle!, 'Temporary');
    
    await TestHelpers.navigateToView(page, 'playlists');
    await page.click('[data-testid="playlist-Temporary"]');
    
    let playlistSongs = await page.locator('[data-testid^="playlist-song-"]').count();
    expect(playlistSongs).toBe(1);
    
    await page.click(`[data-testid="playlist-song-${songTitle}"]`, { button: 'right' });
    await page.click('[data-testid="remove-from-playlist-menu"]');
    
    await page.waitForTimeout(500);
    
    playlistSongs = await page.locator('[data-testid^="playlist-song-"]').count();
    expect(playlistSongs).toBe(0);
    
    const emptyMessage = await page.locator('[data-testid="empty-playlist-message"]');
  ...
  test('should rename playlist', async () => {
    ...
    await page.click('[data-testid="playlist-Original Name"]', { button: 'right' });
    ...
    const oldPlaylist = await page.locator('[data-testid="playlist-Original Name"]').count();
  ...
  test('should delete playlist', async () => {
    ...
    const deletedPlaylist = await page.locator('[data-testid="playlist-To Delete"]').count();
