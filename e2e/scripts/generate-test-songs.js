...
// Realistic test data
const ARTISTS = [
  { name: 'Aurora Synth', albums: ['Digital Dreams', 'Neon Horizons', 'Electric Sunset'] },
  { name: 'The Jazz Collective', albums: ['Blue Notes', 'Midnight Sessions', 'Swing Time'] },
  { name: 'Indie Folk Band', albums: ['Autumn Leaves', 'Mountain Songs', 'Fireside Tales'] },
  { name: 'Electronic Pulse', albums: ['Bass Drop', 'Future Waves', 'Cybernetic'] },
  { name: 'Classical Masters', albums: ['Symphony No. 1', 'Piano Concertos', 'String Quartets'] },
  { name: 'Rock Titans', albums: ['Thunder Road', 'Electric Storm', 'Heavy Metal Heart'] },
  { name: 'Hip Hop Legends', albums: ['Street Poetry', 'Urban Chronicles', 'Beats & Rhymes'] },
  { name: 'Soul Sisters', albums: ['Heartfelt', 'Gospel Light', 'Rhythm & Blues'] },
  { name: 'World Music Ensemble', albums: ['Global Sounds', 'Cultural Fusion', 'Earth Rhythms'] },
  { name: 'Ambient Collective', albums: ['Peaceful Spaces', 'Dreamscapes', 'Meditation'] },
  { name: 'Punk Rockers', albums: ['Anarchy Now', 'Rebellion', 'No Rules'] },
  { name: 'Country Roads', albums: ['Nashville Nights', 'Southern Comfort', 'Dusty Trails'] },
  { name: 'R&B Smooth', albums: ['Late Night Vibes', 'Slow Jams', 'Soulful'] },
  { name: 'Metal Core', albums: ['Destruction', 'Chaos Theory', 'Dark Matter'] },
  { name: 'Reggae Vibes', albums: ['Island Time', 'One Love', 'Roots'] },
  { name: 'Pop Stars', albums: ['Chart Toppers', 'Radio Hits', 'Summer Anthems'] },
  { name: 'Blues Brothers', albums: ['Delta Blues', 'Chicago Nights', 'Twelve Bar'] },
  { name: 'Techno Masters', albums: ['Warehouse', 'Underground', 'Rave Culture'] },
  { name: 'Acoustic Sessions', albums: ['Unplugged', 'Stripped Down', 'Live & Raw'] },
  { name: 'Orchestra Prime', albums: ['Grand Overture', 'Romantic Era', 'Modern Classics'] },
];

const GENRES = [
  'Electronic', 'Jazz', 'Indie Folk', 'EDM', 'Classical', 'Rock',
  'Hip Hop', 'Soul', 'World', 'Ambient', 'Punk', 'Country',
  'R&B', 'Metal', 'Reggae', 'Pop', 'Blues', 'Techno', 'Acoustic', 'Orchestral'
];

const SONG_PREFIXES = [
  'The', 'A', 'My', 'Your', 'Our', 'Lost', 'Found', 'Broken', 'Beautiful', 'Dark',
  'Light', 'Silent', 'Loud', 'Soft', 'Hard', 'Deep', 'High', 'Low', 'Fast', 'Slow'
];

const SONG_NOUNS = [
  'Dream', 'Night', 'Day', 'Love', 'Heart', 'Soul', 'Mind', 'World', 'Sky', 'Ocean',
  'Mountain', 'River', 'Fire', 'Ice', 'Wind', 'Rain', 'Sun', 'Moon', 'Star', 'Light',
  'Shadow', 'Echo', 'Memory', 'Journey', 'Path', 'Road', 'Home', 'Away', 'Time', 'Space',
  'Feeling', 'Moment', 'Story', 'Song', 'Dance', 'Rhythm', 'Beat', 'Melody', 'Harmony', 'Note'
];

const SONG_SUFFIXES = [
  'of Love', 'in the Night', 'at Dawn', 'Forever', 'Again', 'Tonight', 'Tomorrow',
  'Yesterday', 'Always', 'Never', '(Remix)', '(Live)', '(Acoustic)', '(Extended)',
  'Pt. 1', 'Pt. 2', 'Interlude', 'Outro', 'Intro', ''
];

function generateSongTitle(index) {
  const prefix = SONG_PREFIXES[index % SONG_PREFIXES.length];
  const noun = SONG_NOUNS[Math.floor(index / SONG_PREFIXES.length) % SONG_NOUNS.length];
  const suffix = SONG_SUFFIXES[Math.floor(index / (SONG_PREFIXES.length * SONG_NOUNS.length)) % SONG_SUFFIXES.length];
...
function generateMp3(trackData) {
  ...
  const filename = sanitizeFilename(`${paddedIndex} - ${artist} - ${title}.mp3`);
...
`;
  ...
  for (let i = 0; i < validTracks.length; i += batchSize) {
    const batch = validTracks.slice(i, Math.min(i + batchSize, validTracks.length));
    ...
    const values = batch.map((track, batchIndex) => {
      ...
      const lastPlayed = playCount > 0 ? `'2024-${String((globalIndex % 12) + 1).padStart(2, '0')}-${String((globalIndex % 28) + 1).padStart(2, '0')}T00:00:00Z'` : 'NULL';
