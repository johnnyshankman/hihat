...
export class ElectronHelper {
   ...
   */
  static async startDevApp(): Promise<{ app: ElectronApplication; page: Page }> {
   ...
   */
  private static async startRendererServer(env: NodeJS.ProcessEnv): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.devServer.stdout?.on('data', (data) => {
        ...
        if (output.includes('compiled successfully') || output.includes('Compiled with')) {
          resolve();
        }
      });
   ...
   */
  private static async waitForPort(port: number, timeout: number): Promise<void> {
  ...
  private static async waitForWindow(app: ElectronApplication): Promise<Page> {
    ...
    while (retries < maxRetries) {
      try {
        ...
        console.log(`Waiting for window... (attempt ${retries + 1}/${maxRetries})`);
      }
