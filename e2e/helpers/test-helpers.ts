...
export class TestHelpers {
  ...
  static async importSongs(page: Page): Promise<void> {
    ...
    await page.evaluate(async (folderPath) => {
      ...
      if ((window as any).electron && (window as any).electron.library) {
        return await (window as any).electron.library.scan(folderPath);
      } else {
  ...
  static async getPlayerState(page: Page): Promise<{
    ...
    return await page.evaluate(() => {
      ...
      if (!player) return { isPlaying: false, currentSong: null, currentTime: 0, duration: 0 };
      
      const playButton = player.querySelector('[data-testid="play-pause-button"]');
      const isPlaying = playButton?.getAttribute('aria-label') === 'Pause';
      const songTitle = player.querySelector('[data-testid="now-playing-title"]')?.textContent || null;
      const currentTime = parseFloat(player.querySelector('[data-testid="current-time"]')?.textContent || '0');
      const duration = parseFloat(player.querySelector('[data-testid="duration"]')?.textContent || '0');
  ...
  static async navigateToView(page: Page, view: 'library' | 'playlists' | 'settings'): Promise<void> {
  ...
  static async addToPlaylist(page: Page, songTitle: string, playlistName: string): Promise<void> {
    await page.click(`[data-testid="song-row-${songTitle}"]`, { button: 'right' });
  ...
  static async getSongCount(page: Page): Promise<number> {
    const count = await page.locator('[data-testid="song-count"]').textContent();
